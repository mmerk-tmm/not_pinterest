...
class KeywordCRUD(CRUDBase):
    ...
    def toggle_like_keyword(self, user_id: int, keyword_id: int) -> bool:
        liked = self.get_keyword_like_by_user_id(
            user_id=user_id, keyword_id=keyword_id)
        if liked:
            self.delete(model=liked)
            return False
        else:
            self.create(model=KeywordLike(
    ...
    def get_keyword_posts(self, keyword_id: int, page, page_size=20) -> List[Post]:
        ...
        return self.db.query(Post).join(PostKeyword).filter(PostKeyword.keyword_id == keyword_id).order_by(Post.id.desc()).slice(end-page_size, end).all()
