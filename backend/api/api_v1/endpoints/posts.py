...
def get_posts(
    page: int = 1, Auth: Authenticate = Depends(Authenticate(required=False))
):
    posts = PostCRUD(Auth.db).get_posts(page=page)
    for post in posts:
        post.current_user_id = Auth.current_user_id
    return posts


@router.get("/{post_id}", response_model=PostUserWithIdea)
def get_post(post_id: int, Auth: Authenticate = Depends(Authenticate(required=False))):
    post = PostCRUD(Auth.db).get_post_by_id(post_id=post_id)
    if not post:
        raise HTTPException(
...
def update_post(
    ...
    db_post.current_user_id = current_user_id
    return db_post


@router.put("/{post_id}/like", response_model=bool)
def like_post(
    post_id: int, Authorize: AuthJWT = Depends(), db: Session = Depends(get_db)
):
    Authorize.jwt_required()
    current_user_id = Authorize.get_jwt_subject()
    user_cruds = UserCRUD(db)
    ...
    is_liked = post_cruds.toggle_like_post(post_id=post_id, user_id=current_user_id)
    return is_liked
