...
from backend.schemas.schemas import CreateIdea, Idea, IdeaWithPosts, IdeaWithUserAndLike
...
@router.post(
    "",
    response_model=Idea,
    responses={**IDEA_WITH_THIS_NAME_ALREADY_EXISTS_400, **AUTH_REQUIRED_401},
    status_code=status.HTTP_201_CREATED,
)
def create_idea(
    idea_data: CreateIdea, Authorize: AuthJWT = Depends(), db: Session = Depends(get_db)
):
...
def create_post(
    ...
    for keyword in post_data.keywords:
        ...
        keywords.append(db_keyword)
    db_image = save_image(
        upload_file=post_image, db=Auth.db, user_id=Auth.current_user_id
    )
