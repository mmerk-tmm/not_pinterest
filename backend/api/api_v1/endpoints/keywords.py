...
def search_keyword(keyword: str, Auth: Authenticate = Depends(Authenticate())):
    ...
    for keyword in keywords:
        ...
        if Auth.current_user_id:
            keyword_obj.liked = bool(KeywordCRUD(Auth.db).get_keyword_like_by_user_id(
...
def get_keyword_by_id(keyword_id: int, Auth: Authenticate = Depends(Authenticate(required=False))):
...
def delete_keyword_by_id(keyword_id: int, Auth: Authenticate = Depends(Authenticate(is_admin=True))):
    keyword = KeywordCRUD(Auth.db).get_keyword_by_id(keyword_id=keyword_id)
    if not keyword:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="Ключевое слово не найдено")
