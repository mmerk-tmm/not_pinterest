...
const { VITE_MAX_IDEA_NAME_LENGTH, VITE_MAX_IDEA_DESCRIPTION_LENGTH } =
  import.meta.env;
...
const nameAlreadyExists = computed(() => name.value === existingName.value);
...
const buttonActive = computed(
  () =>
    nameIsValid.value && descriptionIsValid.value && !nameAlreadyExists.value
);
const sendIdea = async () => {
  if (!buttonActive.value) return;
  try {
    const { data } = await HTTP.post("ideas", {
      name: name.value,
